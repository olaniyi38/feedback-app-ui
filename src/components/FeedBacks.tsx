...
const FeedBacks = ({ data }: { data: FeedBackType[] }) => {
	...
	return (
		<section>
			<header className=" sticky top-0 md:top-4 z-10 h-full flex gap-y-3 flex-wrap items-end  justify-between p-4 md:rounded-md bg-blue-950 text-sm text-white">
				...
				<div className="flex flex-wrap md:flex-nowrap items-center gap-x-2">
					<label
						htmlFor="#sort"
						className=" basis-[4rem]  pl-1">
						Sort By
					</label>
					<select
						name="sort"
						...
						onChange={updateSort}
						disabled={
							feedbacksFetchStatus === "pending" ||
							feedbacksFetchStatus === "failed"
						}>
