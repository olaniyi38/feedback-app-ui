...
const likeFeedback = createAsyncThunk("feedbacks/like-feedback", async ({ newFeedback, newLikes, id }: {
    newFeedback: FeedBack; newLikes: User['likes']; id: string
}) => {
    const res = await fetch("http://localhost:4000/like-feedback", {
        method: "PATCH",
...
const editFeedback = createAsyncThunk("feedbacks/edit-feebacks", async (data: FeedBack) => {
...
const createFeedback = createAsyncThunk("feedbacks/create-feedback", async (newFeedback: FeedBack) => {
...
const deleteFeedback = createAsyncThunk("feedbacks/deletefeedback", async (id: string): Promise<string> => {
...
const feedbacksSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(fetchFeedbacks.fulfilled, (state, action) => {
            ...
            const feedbackIndex = state.feedbacks.findIndex((fb) => fb.id === action.payload)
